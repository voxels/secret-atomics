...
if (!SANITY_PROJECT_ID || !SANITY_DATASET || !SANITY_TOKEN) {
    console.error('Missing Sanity configuration. Ensure NEXT_PUBLIC_SANITY_PROJECT_ID, NEXT_PUBLIC_SANITY_DATASET, and SANITY_WRITE_TOKEN are set.');
    process.exit(1);
}
...
async function processArticle(url: string) {
    try {
        ...
        if (!dateText) {
            const dateRegex = /(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}(st|nd|rd|th)?,\s+\d{4}/;
        ...
        if (dateText) {
            ...
            if (!isNaN(parsed)) publishDate = new Date(parsed).toISOString().split('T')[0];
        }
        ...
        let heroImageUrl = doc.querySelector('meta[property="og:image"]')?.getAttribute('content');
        ...
        // 3. Content Body
        const contentContainer = doc.querySelector('.post-content') || doc.querySelector('article') || doc.body;
        ...
        for (const b of blocks) {
            ...
            if (block._type === 'block' && block.children && block.children.some((c: any) => c._type === 'image')) {
        ...
        const docData = {
            ...
            body: validBlocks,
            seo: {
                image: heroImageId ? { _type: 'image', asset: { _type: 'reference', _ref: heroImageId } } : undefined,
                description: title
            },
            language: 'en'
        };
