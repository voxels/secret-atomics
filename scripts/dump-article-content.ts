...
async function main() {
    const articles = await client.fetch(`*[_type == "collection.article"] | order(metadata.title asc) {
    ...
    for (const a of articles) {
        ...
        if (!a.body) { console.log('[No body]'); continue; }
        ...
        for (const block of a.body) {
            if (block._type === 'block') {
                ...
                console.log(`[CODE:${block.language || 'text'}] ${(block.code || '').substring(0, 60).replace(/\n/g, '\\n')}...`);
            } else {
