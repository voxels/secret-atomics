...
async function main() {
    const docs = await client.fetch(`*[_type in ["collection.article", "page"]]{ _id, "title": coalesce(metadata.title, title), seo }`);
    console.log(`Updating SEO descriptions for ${Object.keys(SEO_DESCRIPTIONS).length} documents... Mode: ${DRY_RUN ? 'DRY RUN' : 'APPLY'}\n`);
    ...
    for (const doc of docs) {
        ...
        console.log(`   Old (${currentDesc.length} chars): "${currentDesc.substring(0, 60)}${currentDesc.length > 60 ? '...' : ''}"`);
