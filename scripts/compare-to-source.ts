...
function sanityBodyToText(body: any[]): string[] {
    ...
    for (const block of body) {
        if (block._type === 'block') {
            ...
            lines.push(`[CODE:${block.language || 'text'}] ${(block.code || '').substring(0, 80).replace(/\n/g, '\\n')}...`);
...
function htmlToTextLines(html: string): string[] {
    ...
    while (current) {
        ...
        if (text && text.length > 0) {
            ...
            if (text === 'Secret Atomics' || text === 'Blog' || text.startsWith('©') || text.startsWith('Built with')) {
...
function diffTexts(sourceLines: string[], migratedLines: string[]): DiffResult {
    ...
    for (let i = 0; i < normalizedSource.length; i++) {
        if (migratedSet.has(normalizedSource[i])) {
            ...
            const partialMatch = normalizedMigrated.some(m =>
                m.includes(normalizedSource[i]) || normalizedSource[i].includes(m)
            );
            if (!partialMatch) {
    ...
    for (let i = 0; i < normalizedMigrated.length; i++) {
        if (!sourceSet.has(normalizedMigrated[i])) {
            const partialMatch = normalizedSource.some(s =>
                s.includes(normalizedMigrated[i]) || normalizedMigrated[i].includes(s)
            );
            if (!partialMatch) {
...
async function main() {
    ...
    const articles = await client.fetch(`*[_type == "collection.article" && _id match "migrated-*"] | order(metadata.title asc) {
        _id,
    ...
    for (const article of articles) {
        ...
        const matchPct = diff.totalSourceLines > 0
            ? Math.round((diff.matchedLines / diff.totalSourceLines) * 100)
            : 0;

        const icon = matchPct >= 90 ? '✅' : matchPct >= 70 ? '🟡' : '🔴';
        console.log(`${icon} ${title}: ${matchPct}% match (${diff.matchedLines}/${diff.totalSourceLines} source lines found)`);
    ...
    if (poor.length > 0) {
        ...
        for (const p of poor) {
            console.log(`    🔴 ${p.title} (${p.matchPct}%) — ${p.missingFromMigrated.length} lines missing`);
        }
    }
}
