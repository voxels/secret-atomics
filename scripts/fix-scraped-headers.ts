...
async function main() {
    ...
    for (const a of articles) {
        ...
        while (removeCount < a.body.length && getText(a.body[removeCount]) === '' && a.body[removeCount]._type === 'block') {
            removeCount++;
        }

        const removed = a.body.slice(0, removeCount).map((b: any, i: number) => `  [${i}] "${getText(b)}" (${b._type})`);
