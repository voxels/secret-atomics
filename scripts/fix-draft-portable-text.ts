  ...
  for (let i = 0; i < body.length; i++) {
    ...
    if (block._type === 'block' && block.children && Array.isArray(block.children)) {
      ...
      for (const child of block.children) {
        if (child._type === 'span') {
          ...
          } else if (uploadUrl?.startsWith('data:')) {
      ...
      if (extractedBlocks.length > 0) {
        ...
        const hasText = spanChildren.some((c: any) => c.text && c.text.trim().length > 0);
        if (hasText) {
    ...
    if (block._type === 'image' && block._upload_url && !block.asset) {
      ...
      if (resolvedUrl) {
        ...
        changes.push(
          `body[${i}]: top-level image queued for upload: ${resolvedUrl.substring(0, 80)}`
        );
        continue;
      }
      if (block._upload_url.startsWith('data:')) {
...
async function main() {
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(
    `  Summary: ${totalFixed} articles, ${totalChanges} fixes, ${totalUploads} images uploaded`
  );
  if (DRY_RUN) {
