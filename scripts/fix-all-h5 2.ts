...
async function main() {
    ...
    const docs = await client.fetch(`*[defined(body) && count(body[_type == "block" && style in ["h5","h6"]]) > 0]{
        _id, _type, "title": coalesce(metadata.title, title, name), body
    }`);

    console.log(`Found ${docs.length} documents with h5/h6 blocks. Mode: ${DRY_RUN ? 'DRY RUN' : 'APPLY'}\n`);
