 ...
 */
function splitProseAndCode(text: string, codeStart: string): { prose: string | null; code: string } {
...
async function main() {
    ...
    for (const articleId of articleIds) {
        const article = await client.fetch(
            `*[_id == $id][0]{ _id, "title": metadata.title, body }`,
            { id: articleId }
        );
        ...
        for (let i = 0; i < article.body.length; i++) {
            ...
            if (target.codeStartsAt) {
                ...
                console.log(`  ✏️  body[${i}]: text → code (${target.language}): "${text.substring(0, 60)}..."`);
            }
