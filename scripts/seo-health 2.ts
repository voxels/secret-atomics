...
async function main() {
    const docs = await client.fetch(`*[_type in ["collection.article", "page"]] | order(_type, metadata.title) {
        _id, _type,
    ...
    for (const d of docs) {
        ...
        if (!d.seoTitle) {
            ...
            if (d.seoTitle.length < 50) issues.push({ doc: docLabel, level: '🟡', issue: `SEO title too short: ${d.seoTitle.length} chars (aim for 50-60)` });
            if (d.seoTitle.length > 60) issues.push({ doc: docLabel, level: '🟡', issue: `SEO title too long: ${d.seoTitle.length} chars (max 60)` });
        }
        ...
        if (!d.seoDesc) {
            ...
            if (d.seoDesc.length < 70) issues.push({ doc: docLabel, level: '🟡', issue: `SEO description too short: ${d.seoDesc.length} chars (aim for 70-160)` });
            if (d.seoDesc.length > 160) issues.push({ doc: docLabel, level: '🟡', issue: `SEO description too long: ${d.seoDesc.length} chars (max 160)` });
        }
